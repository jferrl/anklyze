...
- [ ] **BUG-03**: Case state transitions (draft → published → closed) are validated in service layer with proper error responses
- [ ] **BUG-04**: Database connection fallback behavior explicitly communicates degraded mode to users and API consumers
...
- [ ] **SEC-04**: Service role key usage is audited and restricted with clear documentation of required permissions

### Architecture

- [ ] **ARCH-01**: ClassificationService wraps rule engine with audit logging, caching, and error handling
- [ ] **ARCH-02**: Study domain service manages case-study relationships transparently
- [ ] **ARCH-03**: Frontend API client implements exponential backoff retry for idempotent operations
- [ ] **ARCH-04**: Case and Study model coupling reduced through domain service abstraction

### Tech Debt — Backend

- [ ] **DEBT-01**: Rule engine suprasyndesmotic AO helper functions consolidated into single parameterized function
- [ ] **DEBT-02**: Statistics service split into focused modules (cohens_kappa.go, fleiss_kappa.go, divergence.go, etc.)
- [ ] **DEBT-03**: LLM prompts extracted from code into external versioned template files
- [ ] **DEBT-04**: JSONB fields validated against schemas before deserialization to prevent silent partial data
- [ ] **DEBT-05**: Database migrations upgraded from GORM AutoMigrate to explicit migration system (golang-migrate or equivalent)

### Tech Debt — Frontend

- [ ] **DEBT-06**: CaseEditorPage decomposed into custom hooks and compound components (target <300 lines per file)
- [ ] **DEBT-07**: AdminCasesPage decomposed into reusable table and filter components
- [ ] **DEBT-08**: AdminStudiesPage decomposed into reusable table and filter components
...
| DEBT-07 | Phase 5 | Pending |
| DEBT-08 | Phase 5 | Pending |
