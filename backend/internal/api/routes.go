...
// SetupDatasetRoutes configures dataset-related routes.
// This should be called after SetupRoutes.
func SetupDatasetRoutes(
	...
	if authValidator != nil {
		setupProtectedDatasetRoutes(api, authValidator, userRepo, datasetHandler)
	} else {
		setupPublicDatasetRoutes(api, datasetHandler)
	}
}

// setupProtectedDatasetRoutes configures dataset routes with authentication.
func setupProtectedDatasetRoutes(
	api *gin.RouterGroup,
	authValidator *auth.Validator,
	userRepo auth.UserService,
	handler *DatasetHandler,
) {
	// Admin-only research dataset routes
	datasets := api.Group("/admin/research/datasets")
	datasets.Use(auth.AuthMiddleware(authValidator))
	datasets.Use(auth.UserSyncMiddleware(userRepo))
	datasets.Use(auth.RequireRole(auth.RoleAdmin))
	{
...
// setupPublicDatasetRoutes configures dataset routes without authentication (development mode).
func setupPublicDatasetRoutes(
	api *gin.RouterGroup,
	handler *DatasetHandler,
) {
	datasets := api.Group("/admin/research/datasets")
